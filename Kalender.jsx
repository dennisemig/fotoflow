...
export default function Kalender() {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 3 }}>
          ...
          {[...Array(daysInMonth)].map((_, i) => {
            ...
            return (
              <div key={day} onClick={() => { setSelectedDate(dayStr); setShowModal(true) }}
                ...
                {daySager.map(s => (
                  <div key={s.id} onClick={e => { e.stopPropagation(); navigate('/sager/' + s.id) }}
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 14, paddingTop: 12, borderTop: '.5px solid var(--brd)', flexWrap: 'wrap', alignItems: 'center' }}>
          {[{ color: '#3A4A5A', label: 'Ny' }, { color: '#2e7d4f', label: 'Aktiv' }, { color: '#e5a243', label: 'Afventer' }, { color: '#0c447c', label: 'Leveret' }, { color: '#6b7280', label: 'Afsluttet' }].map(l => (
      ...
      {showModal && <OpretSagModal dato={selectedDate} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); fetchSager(); toast('✓ Sag oprettet!') }} toast={toast} />}
    </div>
  )
}
...
function OpretSagModal({ dato, onClose, onSaved, toast }) {
  ...
  const datoLabel = dato ? new Date(dato).toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : ''

  async function handleSave() {
    if (!form.adresse) { toast('Udfyld adresse', 'error'); return }
    setSaving(true)
    const { error } = await supabase.from('sager').insert([{
      adresse: form.adresse, dato: dato, tidspunkt: form.tidspunkt || null,
      type: form.type, kunde_id: form.kunde_id || null,
      freelancer_id: form.freelancer_id || null,
      maks_billeder: form.maks_billeder, noter: form.noter || null, status: 'ny'
    }])
  ...
  return (
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-title">Opret sag – {datoLabel}<button className="modal-close" onClick={onClose}>✕</button></div>
