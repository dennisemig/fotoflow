  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="card"><div className="empty-state"><div className="empty-icon">📷</div>Ingen freelancere endnu – klik "+ Tilføj freelancer"</div></div>
      ) : (
        <div className="grid2">
          {filtered.map((f, i) => (
            <div key={f.id}
              onClick={() => navigate(`/freelancere/${f.id}`)}
              ...
              <span className={`badge ${f.aktiv !== false ? 'badge-active' : 'badge-new'}`}>
                {f.aktiv !== false ? 'Aktiv' : 'Inaktiv'}
              </span>
            </div>
          ))}
        </div>
      ...
      {showModal && (
        <TilfoejFreelancerModal
          onClose={() => setShowModal(false)}
          onSaved={() => { setShowModal(false); fetch(); toast('✓ Freelancer tilføjet!') }}
          toast={toast}
        />
      )}
    </div>
  ...
  return (
    <div className="modal-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        {step === 3 && (
          <>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 14 }}>{form.navn} modtager en invitationsmail:</div>
            <div style={{ background: 'var(--bg)', borderRadius: 10, padding: 16, border: '.5px solid var(--brd)', marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 13, lineHeight: 1.6 }}>Hej {form.navn},<br /><br />Du er tilføjet som freelancerfotograf hos VaniaGraphics.<br /><br /><span style={{ color: 'var(--blu)', textDecoration: 'underline' }}>Opret din adgang →</span></div>
            </div>
          </>
        )}
