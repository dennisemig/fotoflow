...
export default function Sager() {
  ...
  async function fetchSager() {
    ...
    const { data, error } = await supabase
      .from('sager')
      .select('*, kunder(navn, email, telefon)')
      .order('created_at', { ascending: false })
    if (error) console.error('Fejl ved hentning:', error)

    if (data && data.length > 0) {
      const freelancerIds = data.filter(s => s.freelancer_id).map(s => s.freelancer_id)
      let freelancerMap = {}
      if (freelancerIds.length > 0) {
        const { data: flData } = await supabase
          ...
          .in('id', freelancerIds)
        if (flData) flData.forEach(f => { freelancerMap[f.id] = f })
      }
      const merged = data.map(s => ({ ...s, freelancer: freelancerMap[s.freelancer_id] || null }))
      setSager(merged)
    } else {
      setSager(data || [])
    }
      ...
      <div className="card">
        {loading ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table>
              <thead>
                <tr><th>Kunde</th><th>Adresse</th><th>Dato</th><th>Type</th><th>Freelancer</th><th>Status</th><th>Handling</th></tr>
              </thead>
              <tbody>
                {filtered.map(s => (
...
function OpretSagModal({ onClose, onSaved, toast }) {
  const [form, setForm] = useState({
    adresse: '', dato: '', tidspunkt: '10:00', type: 'ejendom',
    freelancer_id: '', kunde_id: '', maks_billeder: 20, noter: '', beskrivelse: ''
  })
  ...
  useEffect(() => {
    ...
    supabase.from('freelancere').select('id, navn, email').eq('aktiv', true).then(({ data }) => setFreelancere(data || []))
  }, [])
  ...
  async function lookupBBR(adresse) {
    ...
    try {
      ...
      if (d.length > 0) {
        setBbr({ vejnavn: d[0].vejnavn, postnr: d[0].postnr })
      }
    } catch (e) { }
    setBbrLoading(false)
  }
  ...
  async function handleSave() {
    ...
    if (error) {
      ...
      toast('Fejl: ' + error.message, 'error')
      setSaving(false)
      return
    }
    ...
    if (form.freelancer_id) {
      ...
      await fetch('/api/send-notification', {
        ...
        body: JSON.stringify({
          ...
          mægler: { email: fl?.email, navn: fl?.navn, adresse: form.adresse, dato: new Date(form.dato).toLocaleDateString('da-DK'), tidspunkt: form.tidspunkt }
        })
      }).catch(() => { })
    }
    setSaving(false)
      ...
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <input value={form.adresse} onChange={e => set('adresse', e.target.value)}
            onBlur={e => lookupBBR(e.target.value)} placeholder="f.eks. Lyngvigvej 12, 2750 Ballerup" autoFocus />
          {bbrLoading && <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>⏳ Henter BBR-data...</div>}
          {bbr && <div style={{ fontSize: 12, color: 'var(--grn)', marginTop: 4 }}>✓ Adresse verificeret</div>}
        </div>
        <div className="form-group">
